...
from helpers.strike_coordination import StrikeCoordinator
import os
import logging
...
class Strategy:
    ...
    def _opposite_leg_move_to_cost_active(self):
        ...
        try:
            ...
            while True:
                ...
                if (start_time <= current_time <= closing_time) or self.testing:
                    ...
                    strike_step = credentials.XSP_SPX_STRIKE
                    ...
                    if credentials.calc_values:
                        self.otm_closest_call = closest_strike + (credentials.OTM_CALL_HEDGE * strike_step)
                    await self.dprint(f"CALL HEDGE STRIKE PRICE: {self.otm_closest_call}")

                    if credentials.calc_values:
                        self.otm_closest_put = closest_strike - (credentials.OTM_PUT_HEDGE * strike_step)
                    ...
                    if credentials.calc_values:
                        ...
                        if credentials.ATM_CALL > 0:
                            self.call_target_price += strike_step * credentials.ATM_CALL
                    await self.dprint(f"CALL POSITION STRIKE PRICE: {self.call_target_price}")

                    if credentials.calc_values:
                        self.put_target_price = closest_strike
                        if credentials.ATM_CALL > 0:
                            self.put_target_price -= strike_step * credentials.ATM_CALL
            ...
            if hasattr(self, "call_target_price") and self.call_target_price is not None:
                await StrikeCoordinator.remove_active_strike(self, "calls", self.call_target_price)
            if hasattr(self, "put_target_price") and self.put_target_price is not None:
                await StrikeCoordinator.remove_active_strike(self, "puts", self.put_target_price)

...
class IBTWSAPI:
    ...
    async def fetch_strikes(self, symbol, exchange, secType='STK'):
        ...
        if secType == 'FUT':
            ...
            qualified = self.client.qualifyContracts(fut_contract)
            if qualified:
                base_contract = qualified[0]
                self.client.reqMarketDataType(1)
                chains = self.client.reqSecDefOptParams(
                    base_contract.symbol, '', base_contract.secType, base_contract.conId
                )
                chain = next(
                    (
                        c for c in chains
                        if c.exchange == exchange and c.tradingClass == credentials.tradingClass
                    ),
                    None
                )
            ...
            # Fallback: filter all FOP strikes by option expiry date if month chain lookup fails
            ...
            if not strikes:
                raise ValueError(
                    f"No strikes found for future month {future_month} or option expiry {credentials.date}"
                )
            return strikes
    ...
    async def current_price(self, symbol, exchange='CBOE'):
        futures_exchanges = {"NYMEX", "CME", "COMEX", "CBOT"}
        if symbol in {"CL", "NG"} or exchange in futures_exchanges:
            future_month = getattr(credentials, "future_date", None) or credentials.date[:6]
            spx_contract = Future(
                symbol=symbol,
                lastTradeDateOrContractMonth=future_month,

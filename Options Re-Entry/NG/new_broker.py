...
class IBTWSAPI:
        ...
        if contract == "stocks":
            ...
            if str(symbol).upper() == "NG":
                return FuturesOption(
                    ...
                    strike=strike,
                    right=right,
                    multiplier=credentials.option_multiplier,
            ...
            return Option(symbol=symbol, lastTradeDateOrContractMonth=expiry, exchange=exchange, currency="USD",
                          strike=strike, right=right, multiplier="100", tradingClass=credentials.tradingClass)
    ...
    async def get_expiries_and_strikes(self, technology: str, ticker: str) -> dict:
        ...
        """
        # Creating contract
        if technology.lower() == "options" and str(ticker).upper() != "NG":
            c = Option()
        else:
            c = FuturesOption()
        c.symbol = ticker
        c.strike = ""
        c.lastTradeDateOrContractMonth = ""
        if str(ticker).upper() == "NG":
            c.exchange = credentials.exchange
            c.currency = credentials.currency
            c.multiplier = credentials.option_multiplier
            c.tradingClass = credentials.tradingClass
        contract_info = self.client.reqContractDetails(contract=c)
        # print(contract_info)
    ...
    async def fetch_strikes(self, symbol, exchange, secType='STK'):
        ...
        if secType == 'FUT':
            expiries_and_strikes = await self.get_expiries_and_strikes("futureOptions", symbol)
            target_expiry = dt.datetime.strptime(credentials.date, "%Y%m%d").date()
            strikes = expiries_and_strikes.get(target_expiry)
            if not strikes:
                raise ValueError(f"No strikes found for expiry {credentials.date}")
            return strikes
        ...
        if secType == 'IND':
            ...
            raise ValueError(f"Unsupported secType: {secType}. Use 'IND', 'FUT' or 'STK'.")
    ...
    async def current_price(self, symbol, exchange='CBOE'):
        if str(symbol).upper() == "NG":
            future_month = getattr(credentials, "future_date", None) or credentials.date[:6]
            spx_contract = Future(
                symbol=symbol,
                lastTradeDateOrContractMonth=future_month,
                exchange=exchange,
    ...
    async def cancel_hedge(self):
        ...
        for position in positions:
            if position.position > 0:
                ...
                if str(position.contract.symbol).upper() == "NG":
    ...
    async def cancel_call(self, hedge_strike, position_strike, close_hedge):
        hedge_contract = self._create_contract("options", credentials.instrument, credentials.exchange,
                                               credentials.date, hedge_strike, "C")
        contract = self._create_contract("options", credentials.instrument, credentials.exchange,
                                         credentials.date, position_strike, "C")
    ...
    async def cancel_put(self, hedge_strike, position_strike, close_hedge):
        hedge_contract = self._create_contract("options", credentials.instrument, credentials.exchange,
                                               credentials.date, hedge_strike, "P")
        contract = self._create_contract("options", credentials.instrument, credentials.exchange,
                                         credentials.date, position_strike, "P")
    ...
    async def get_latest_premium_price(self, symbol, expiry, strike, right, exchange=credentials.exchange, print_data=True):

        option_contract = FuturesOption(
            symbol=symbol,
            lastTradeDateOrContractMonth=expiry,
            strike=strike,
            right=right,
    ...
    async def place_stp_order(self, contract, side, quantity, sl):
        ...
        stop_order = StopOrder(side, quantity, round(sl, 3))
        stop_order.tif = "DAY"
    ...
    async def modify_stp_order(self, contract, quantity, side, sl, order_id):
        ...
        stop_order = StopOrder(side, quantity, round(sl, 3), orderId=order_id)
        stop_order.tif="DAY"

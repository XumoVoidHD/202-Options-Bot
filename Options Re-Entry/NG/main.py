...
class Strategy:
    ...
    @staticmethod
    def round_to_valid_ng_strike(price):
        return math.floor((price * 20) + 0.5) / 20
    ...
    async def main(self):
        ...
        while True:
            ...
            if (start_time <= current_time <= closing_time) or self.testing:
                ...
                closest_strike = self.round_to_valid_ng_strike(current_price)
                ...
                if credentials.calc_values:
                    self.otm_closest_call = closest_strike + (credentials.OTM_CALL_HEDGE * 0.05)
                await self.dprint(f"CALL HEDGE STRIKE PRICE: {self.otm_closest_call}")

                if credentials.calc_values:
                    self.otm_closest_put = closest_strike - (credentials.OTM_PUT_HEDGE * 0.05)
    ...
    async def place_hedge_orders(self, call, put):
        if call:
            spx_contract_call = FuturesOption(
        ...
        if put:
            spx_contract_put = FuturesOption(
                symbol=credentials.instrument,
                lastTradeDateOrContractMonth=credentials.date,
    ...
    async def place_atm_call_order(self):
        ...
        self.call_contract = FuturesOption(
            symbol=credentials.instrument,
            lastTradeDateOrContractMonth=credentials.date,
            strike=self.call_target_price,
            right='C',
    ...
    async def call_hedge_check(self):
        while self.should_continue:
            if self.call_order_placed:
                ...
                call_exists = any(
                    trade.contract.secType == 'FOP' and trade.contract.right == 'C' and
    ...
    async def place_atm_put_order(self):
        ...
        self.put_contract = FuturesOption(
            symbol=credentials.instrument,
            lastTradeDateOrContractMonth=credentials.date,
            strike=self.put_target_price,
            right='P',
    ...
    async def put_hedge_check(self):
        while self.should_continue:
            if self.put_order_placed:
                ...
                put_exists = any(
                    trade.contract.secType == 'FOP' and trade.contract.right == 'P' and
